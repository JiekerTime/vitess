...
func TestMain(m *testing.M) {
	...
	exitCode := func() int {
		...
		primaryTabletParams = mysql.ConnParams{
			Uname:      username,
			DbName:     dbName,
			UnixSocket: path.Join(os.Getenv("VTDATAROOT"), fmt.Sprintf("/vt_%010d/tmp/mysql.socket", primaryTablet.TabletUID)),
		}
		replicaTabletParams = mysql.ConnParams{
			Uname:      username,
			DbName:     dbName,
			UnixSocket: path.Join(os.Getenv("VTDATAROOT"), fmt.Sprintf("/vt_%010d/tmp/mysql.socket", replicaTablet.TabletUID)),
		}
