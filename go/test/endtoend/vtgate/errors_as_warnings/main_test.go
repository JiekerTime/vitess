...
func TestScatterErrsAsWarns(t *testing.T) {
	...
	for _, mode := range modes {
		t.Run(mode.m, func(t *testing.T) {
			...
			assertContainsOneOf(t, mode.conn, showQ, "operation not allowed in state SHUTTING_DOWN", "no valid tablet", "no healthy tablet", "mysql.socket: connect: no such file or directory")
