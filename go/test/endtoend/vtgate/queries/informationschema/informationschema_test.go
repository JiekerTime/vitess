...
func TestDbNameOverride(t *testing.T) {
	...
	assert.Equal(t, topoproto.VtDbPrefix+"ks", qr.Rows[0][0].ToString())
}
...
func TestInformationSchemaQuery(t *testing.T) {
	...
	utils.AssertSingleRowIsReturned(t, mcmp.VtConn, "table_schema = 'ks'", topoproto.VtDbPrefix+"ks")
	...
	utils.AssertSingleRowIsReturned(t, mcmp.VtConn, "table_schema = 'ks' and table_name = 't1'", topoproto.VtDbPrefix+"ks")
}
