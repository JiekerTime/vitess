...
func TestTableUnion(t *testing.T) {
	...
	mcmp.ExecWithColumnCompare("(SELECT f_int FROM t_user ORDER BY f_tinyint DESC LIMIT 1) UNION ALL (SELECT f_int FROM t_1 ORDER BY f_table DESC LIMIT 1)")
	...
	mcmp.ExecWithColumnCompare("(select f_int from t_user order by intcol limit 5) union all (select f_int from t_1 order by f_table desc limit 5)")
