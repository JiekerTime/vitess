...
func TestTableUnion(t *testing.T) {
	...
	mcmp.Exec("insert into t_11(f_shard,f_table,f_int) values (61,'c',21),(62,'d',22),(63,'e',23),(64,'f',24);")
	...
	mcmp.ExecAndNotEmpty("select * from (select * from t_9 union all select * from t_11) as t")
	// union with different target shards
	mcmp.ExecAndNotEmpty("select 1 from t_9 where id = 1 union select 1 from t_11 where id = 2")
