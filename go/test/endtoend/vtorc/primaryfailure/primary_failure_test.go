...
func TestLostRdonlyOnPrimaryFailure(t *testing.T) {
	...
	out, err := utils.RunSQL(t, "SELECT * FROM vt_insert_test", replica, "ks")
	require.NoError(t, err)
	require.Equal(t, 1, len(out.Rows))
	out, err = utils.RunSQL(t, "SELECT * FROM vt_insert_test", rdonly, "ks")
	require.NoError(t, err)
	require.Equal(t, 1, len(out.Rows))
...
func TestDownPrimaryPromotionRuleWithLag(t *testing.T) {
	...
	out, err := utils.RunSQL(t, "SELECT * FROM vt_insert_test", crossCellReplica, "ks")
	require.NoError(t, err)
	require.Equal(t, 1, len(out.Rows))
