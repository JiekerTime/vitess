...
func registerMyCnfFlags(fs *pflag.FlagSet) {
	...
	fs.BoolVar(&dockerRun, "docker_run", dockerRun, "Use docker to run service. If run in the docker remove the uid from the configuration director")
}
...
func MycnfFile(uid uint32) string {
	if dockerRun {
		mr, _ := env.VtMysqlRoot()
		return path.Join(mr, "etc/my.cnf")
	}
