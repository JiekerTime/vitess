...
func (m *Keyspace) CloneVT() *Keyspace {
	...
	r := &Keyspace{
		...
		CrossTablet:            m.CrossTablet,
		AttachEnable:           m.AttachEnable,
		AttachTo:               m.AttachTo,
	}
	...
	if rhs := m.SplittableTables; rhs != nil {
		tmpContainer := make(map[string]*SplitTable, len(rhs))
		for k, v := range rhs {
			tmpContainer[k] = v.CloneVT()
		}
		r.SplittableTables = tmpContainer
	}
	if rhs := m.SplittableVindexes; rhs != nil {
		tmpContainer := make(map[string]*Vindex, len(rhs))
		for k, v := range rhs {
			tmpContainer[k] = v.CloneVT()
		}
		r.SplittableVindexes = tmpContainer
	}
	if len(m.unknownFields) > 0 {
		r.unknownFields = make([]byte, len(m.unknownFields))
		copy(r.unknownFields, m.unknownFields)
...
func (m *SplitTable) CloneVT() *SplitTable {
	if m == nil {
		return (*SplitTable)(nil)
	}
	r := &SplitTable{
		LogicTableName: m.LogicTableName,
		TableVindex:    m.TableVindex,
		TableCount:     m.TableCount,
	}
	if rhs := m.TableVindexColumn; rhs != nil {
		tmpContainer := make([]*TableVindexColumn, len(rhs))
		for k, v := range rhs {
			tmpContainer[k] = v.CloneVT()
		}
		r.TableVindexColumn = tmpContainer
	}
	if rhs := m.Params; rhs != nil {
		tmpContainer := make(map[string]string, len(rhs))
		for k, v := range rhs {
...
func (m *SplitTable) CloneMessageVT() proto.Message {
	return m.CloneVT()
}

func (m *TableVindexColumn) CloneVT() *TableVindexColumn {
	if m == nil {
		return (*TableVindexColumn)(nil)
	}
	r := &TableVindexColumn{
		Index:      m.Index,
		Column:     m.Column,
...
func (m *TableVindexColumn) CloneMessageVT() proto.Message {
	return m.CloneVT()
}
...
func (m *Keyspace) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.SplittableVindexes) > 0 {
		for k := range m.SplittableVindexes {
			v := m.SplittableVindexes[k]
			baseI := i
	...
	if len(m.SplittableTables) > 0 {
		for k := range m.SplittableTables {
			v := m.SplittableTables[k]
			baseI := i
			size, err := v.MarshalToSizedBufferVT(dAtA[:i])
			if err != nil {
				return 0, err
...
func (m *SplitTable) MarshalVT() (dAtA []byte, err error) {
	if m == nil {
		return nil, nil
	}
	size := m.SizeVT()
...
func (m *SplitTable) MarshalToVT(dAtA []byte) (int, error) {
	size := m.SizeVT()
	return m.MarshalToSizedBufferVT(dAtA[:size])
}

func (m *SplitTable) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
	...
	if m.TableCount != 0 {
		i = encodeVarint(dAtA, i, uint64(m.TableCount))
		i--
		dAtA[i] = 0x20
	}
	...
	if len(m.LogicTableName) > 0 {
		i -= len(m.LogicTableName)
		copy(dAtA[i:], m.LogicTableName)
		i = encodeVarint(dAtA, i, uint64(len(m.LogicTableName)))
		i--
		dAtA[i] = 0xa
	}
	return len(dAtA) - i, nil
...
func (m *Keyspace) SizeVT() (n int) {
	...
	if len(m.SplittableTables) > 0 {
		for k, v := range m.SplittableTables {
			_ = k
			_ = v
			l = 0
			if v != nil {
	...
	if len(m.SplittableVindexes) > 0 {
		for k, v := range m.SplittableVindexes {
			_ = k
			_ = v
			l = 0
			if v != nil {
...
func (m *SplitTable) SizeVT() (n int) {
	if m == nil {
		return 0
	}
	var l int
	_ = l
	l = len(m.LogicTableName)
	if l > 0 {
		n += 1 + l + sov(uint64(l))
	}
	l = len(m.TableVindex)
...
func (m *Keyspace) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 88:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field SplittableTables", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if m.SplittableTables == nil {
				m.SplittableTables = make(map[string]*SplitTable)
			}
			var mapkey string
			var mapvalue *SplitTable
			for iNdEx < postIndex {
				entryPreIndex := iNdEx
				var wire uint64
				for shift := uint(0); ; shift += 7 {
			...
			m.SplittableTables[mapkey] = mapvalue
			iNdEx = postIndex
		case 89:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field SplittableVindexes", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if m.SplittableVindexes == nil {
				m.SplittableVindexes = make(map[string]*Vindex)
			}
			var mapkey string
			var mapvalue *Vindex
			for iNdEx < postIndex {
			...
			m.SplittableVindexes[mapkey] = mapvalue
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := skip(dAtA[iNdEx:])
...
func (m *SplitTable) UnmarshalVT(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		...
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: SplitTable: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: SplitTable: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field LogicTableName", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.LogicTableName = string(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		case 2:
			if wireType != 2 {
