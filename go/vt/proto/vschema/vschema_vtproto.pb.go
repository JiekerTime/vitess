...
func (m *Keyspace) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.SplitTables) > 0 {
		for k := range m.SplitTables {
			v := m.SplitTables[k]
			baseI := i
...
func (m *Keyspace) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 88:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field SplitTables", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if m.SplitTables == nil {
				m.SplitTables = make(map[string]*Table)
			}
			var mapkey string
			var mapvalue *Table
			for iNdEx < postIndex {
