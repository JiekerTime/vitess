...
// ResolvedTable contains everything we need to send a query to a shard.
...
// ResolvedTableEqual is an equality check on *ResolvedShard.
func ResolvedTableEqual(rs1, rs2 *ResolvedShard) bool {
	return proto.Equal(rs1.Target, rs2.Target)
}

// ResolvedTablesEqual is an equality check on []*ResolvedShard.
func ResolvedTablesEqual(rss1, rss2 []*ResolvedShard) bool {
...
func (rs *ResolvedTable) WithKeyspace(newKeyspace string) *ResolvedShard {
...
// A TableResolver can resolve table ids and key ranges into ResolvedTable*
...
func (r *TableResolver) GetKeyspaceShards(ctx context.Context, keyspace string, tabletType topodatapb.TabletType) (string, *topodatapb.SrvKeyspace, []*topodatapb.ShardReference, error) {
...
// and GetSrvKeyspace.
func (r *TableResolver) GetAllShards(ctx context.Context, keyspace string, tabletType topodatapb.TabletType) ([]*ResolvedShard, *topodatapb.SrvKeyspace, error) {
...
func (r *TableResolver) GetAllKeyspaces(ctx context.Context) ([]string, error) {
...
func (r *TableResolver) ResolveDestinationsMultiCol(ctx context.Context, keyspace string, tabletType topodatapb.TabletType, ids [][]sqltypes.Value, destinations []key.Destination) ([]*ResolvedTable, [][][]sqltypes.Value, error) {
	keyspace, _, allShards, err := r.GetKeyspaceShards(ctx, keyspace, tabletType)
	if err != nil {
		return nil, nil, err
	}
	accumulator := &resultTableAcc{
	...
	for _, destination := range destinations {
		if err := destination.Resolve(allShards, nil); err != nil {
			return nil, nil, err
		}
	}
	return accumulator.tables, accumulator.values, nil
}

type resultTableAcc struct {
...
func (r *resultTableAcc) ResolveDestinations(ctx context.Context, keyspace string, tabletType topodatapb.TabletType, ids []*querypb.Value, destinations []key.Destination) ([]*ResolvedShard, [][]*querypb.Value, error) {
	//keyspace, _, allShards, err := r.GetKeyspaceTables(ctx, keyspace, tabletType)
	//if err != nil {
	//	return nil, nil, err
	//}

	var result []*resultTableAcc
	var values [][]*querypb.Value
	resolved := make(map[string]int)
	for i, destination := range destinations {
		if err := destination.Resolve(nil, func(shard string) error {
			s, ok := resolved[shard]
			if !ok {
				target := &querypb.Target{
					Keyspace:   keyspace,
...
func (r *resultTableAcc) ResolveDestination(ctx context.Context, keyspace string, tabletType topodatapb.TabletType, destination key.Destination) ([]*ResolvedShard, error) {
