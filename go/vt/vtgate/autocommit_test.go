...
func TestAutocommitTransactionStarted(t *testing.T) {
	...
	sql := "update `user` set a = 2 where id = 1"
	_, err := executor.Execute(context.Background(), nil, nil, "TestExecute", NewSafeSession(session), sql, map[string]*querypb.BindVariable{})
	require.NoError(t, err)
...
func autocommitExec(executor *Executor, sql string) (*sqltypes.Result, error) {
	...
	return executor.Execute(context.Background(), nil, nil, "TestExecute", NewSafeSession(session), sql, map[string]*querypb.BindVariable{})
}
