...
func TestReservedConnDML(t *testing.T) {
	...
	_, err := executor.Execute(ctx, nil, nil, "TestReservedConnDML", session, "use "+KsTestUnsharded, nil)
	require.NoError(t, err)
	...
	_, err = executor.Execute(ctx, nil, nil, "TestReservedConnDML", session, "set default_week_format = 1", nil)
	require.NoError(t, err)
	assertQueries(t, sbc, wantQueries)

	_, err = executor.Execute(ctx, nil, nil, "TestReservedConnDML", session, "begin", nil)
	require.NoError(t, err)
	...
	_, err = executor.Execute(ctx, nil, nil, "TestReservedConnDML", session, "insert into `simple`() values ()", nil)
	require.NoError(t, err)
	assertQueries(t, sbc, wantQueries)

	_, err = executor.Execute(ctx, nil, nil, "TestReservedConnDML", session, "commit", nil)
	require.NoError(t, err)
...
func TestStreamingDML(t *testing.T) {
	...
	for _, tcase := range tcases {
		...
		err := executor.StreamExecute(ctx, nil, nil, method, session, tcase.query, nil, func(result *sqltypes.Result) error {
			qr = result
			return nil
		})
		require.NoError(t, err)
...
func TestInsertSelectFromDual(t *testing.T) {
	...
	for _, workload := range []string{"olap", "oltp"} {
		t.Run(workload, func(t *testing.T) {
			...
			_, err := executor.Execute(context.Background(), nil, nil, "TestInsertSelect", session, wQuery, nil)
			require.NoError(t, err)

			_, err = executor.Execute(context.Background(), nil, nil, "TestInsertSelect", session, query, nil)
			require.NoError(t, err)
