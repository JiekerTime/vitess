...
func TestExecutorResultsExceeded(t *testing.T) {
	...
	_, err := executor.Execute(ctx, nil, nil, "TestExecutorResultsExceeded", session, "select * from main1", nil)
...
func TestExecutorMaxMemoryRowsExceeded(t *testing.T) {
	...
	for _, test := range testCases {
		...
		_, err = executor.Execute(ctx, nil, nil, "TestExecutorMaxMemoryRowsExceeded", session, test.query, nil)
		...
		sbclookup.SetResults([]*sqltypes.Result{result})
		err = executor.StreamExecute(ctx, nil, nil, "TestExecutorMaxMemoryRowsExceeded", session, test.query, nil, fn)
...
func TestExecutorTransactionsNoAutoCommit(t *testing.T) {
	...
	// begin.
	_, err := executor.Execute(ctx, nil, nil, "TestExecute", session, "begin", nil)
	require.NoError(t, err)
	...
	// commit.
	_, err = executor.Execute(ctx, nil, nil, "TestExecute", session, "select id from main1", nil)
	require.NoError(t, err)
	logStats = testQueryLog(t, executor, logChan, "TestExecute", "SELECT", "select id from main1", 1)
	assert.EqualValues(t, 0, logStats.CommitTime, "logstats: expected zero CommitTime")
	assert.EqualValues(t, "suuid", logStats.SessionUUID, "logstats: expected non-empty SessionUUID")

	_, err = executor.Execute(context.Background(), nil, nil, "TestExecute", session, "commit", nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	_, err = executor.Execute(ctx, nil, nil, "TestExecute", session, "select id from main1", nil)
	require.NoError(t, err)
	_, err = executor.Execute(ctx, nil, nil, "TestExecute", session, "rollback", nil)
	require.NoError(t, err)
	wantSession = &vtgatepb.Session{TargetString: "@primary", SessionUUID: "suuid"}
	utils.MustMatch(t, wantSession, session.Session, "session")
	...
	_, err = executor.Execute(ctx, nil, nil, "TestExecute", session, "use @replica", nil)
...
func TestDirectTargetRewrites(t *testing.T) {
	...
	_, err := executor.Execute(ctx, nil, nil, "TestExecute", NewSafeSession(session), sql, map[string]*querypb.BindVariable{})
	require.NoError(t, err)
...
func TestExecutorTransactionsAutoCommit(t *testing.T) {
	...
	_, err = executor.Execute(ctx, nil, nil, "TestExecute", session, "select id from main1", nil)
	require.NoError(t, err)
	_, err = executor.Execute(ctx, nil, nil, "TestExecute", session, "commit", nil)
	require.NoError(t, err)
...
func TestExecutorTransactionsAutoCommitStreaming(t *testing.T) {
	...
	// begin.
	err := executor.StreamExecute(ctx, nil, nil, "TestExecute", session, "begin", nil, func(result *sqltypes.Result) error {
...
func TestExecutorDeleteMetadata(t *testing.T) {
	...
	_, err := executor.Execute(ctx, nil, nil, "TestExecute", session, set, nil)
	assert.NoError(t, err, "%s error: %v", set, err)

	show := `show vitess_metadata variables like 'app\\_%'`
	result, _ := executor.Execute(ctx, nil, nil, "TestExecute", session, show, nil)
	assert.Len(t, result.Rows, 1)

	// Fails if deleting key that doesn't exist
	delQuery := "set @@vitess_metadata.doesn't_exist=''"
	_, err = executor.Execute(ctx, nil, nil, "TestExecute", session, delQuery, nil)
	...
	show = `show vitess_metadata variables like 'app\\_%'`
	_, err = executor.Execute(ctx, nil, nil, "TestExecute", session, show, nil)
	assert.True(t, topo.IsErrType(err, topo.NoNode))
}
...
func TestExecutorAutocommit(t *testing.T) {
	...
	// autocommit = 1
	_, err = executor.Execute(ctx, nil, nil, "TestExecute", session, "set autocommit=1", nil)
	require.NoError(t, err)
	...
	_, err = executor.Execute(ctx, nil, nil, "TestExecute", session, "update main1 set id=1", nil)
	require.NoError(t, err)
...
func TestExecutorShow(t *testing.T) {
	...
	_, err := executor.Execute(ctx, nil, nil, "TestExecute", session, "show variables", nil)
	require.NoError(t, err)
	_, err = executor.Execute(ctx, nil, nil, "TestExecute", session, "show collation", nil)
	require.NoError(t, err)
	_, err = executor.Execute(ctx, nil, nil, "TestExecute", session, "show collation where `Charset` = 'utf8' and `Collation` = 'utf8_bin'", nil)
	require.NoError(t, err)

	_, err = executor.Execute(ctx, nil, nil, "TestExecute", session, "use @primary", nil)
	require.NoError(t, err)
	_, err = executor.Execute(ctx, nil, nil, "TestExecute", session, "show tables", nil)
	...
	_, err = executor.Execute(ctx, nil, nil, "TestExecute", session, "show create table unknown_table", nil)
	assert.EqualErrorf(t, err, wantErrNoTable, "Got: %v. Want: %v", wantErrNoTable)

	// SHOW CREATE table using vschema to find keyspace.
	_, err = executor.Execute(ctx, nil, nil, "TestExecute", session, "show create table user_seq", nil)
	require.NoError(t, err)
	lastQuery = sbclookup.Queries[len(sbclookup.Queries)-1].Sql
	...
	_, err = executor.Execute(ctx, nil, nil, "TestExecute", session, fmt.Sprintf("show create table %v.unknown", KsTestUnsharded), nil)
	require.NoError(t, err)
	lastQuery = sbclookup.Queries[len(sbclookup.Queries)-1].Sql
	...
	_, err = executor.Execute(ctx, nil, nil, "TestExecute", session, fmt.Sprintf("show keys from %v.unknown", KsTestUnsharded), nil)
	require.NoError(t, err)
	lastQuery = sbclookup.Queries[len(sbclookup.Queries)-1].Sql
	wantQuery = "show indexes from unknown"
	assert.Equal(t, wantQuery, lastQuery, "Got: %v. Want: %v", lastQuery, wantQuery)

	_, err = executor.Execute(ctx, nil, nil, "TestExecute", session, fmt.Sprintf("show keys from unknown from %v", KsTestUnsharded), nil)
	require.NoError(t, err)
	lastQuery = sbclookup.Queries[len(sbclookup.Queries)-1].Sql
	assert.Equal(t, wantQuery, lastQuery, "Got: %v. Want: %v", lastQuery, wantQuery)

	// SHOW INDEX with two different syntax
	_, err = executor.Execute(ctx, nil, nil, "TestExecute", session, fmt.Sprintf("show index from %v.unknown", KsTestUnsharded), nil)
	require.NoError(t, err)
	lastQuery = sbclookup.Queries[len(sbclookup.Queries)-1].Sql
	assert.Equal(t, wantQuery, lastQuery, "Got: %v. Want: %v", lastQuery, wantQuery)

	_, err = executor.Execute(ctx, nil, nil, "TestExecute", session, fmt.Sprintf("show index from unknown from %v", KsTestUnsharded), nil)
	require.NoError(t, err)
	lastQuery = sbclookup.Queries[len(sbclookup.Queries)-1].Sql
	assert.Equal(t, wantQuery, lastQuery, "Got: %v. Want: %v", lastQuery, wantQuery)

	// SHOW INDEXES with two different syntax
	_, err = executor.Execute(ctx, nil, nil, "TestExecute", session, fmt.Sprintf("show indexes from %v.unknown", KsTestUnsharded), nil)
	require.NoError(t, err)
	lastQuery = sbclookup.Queries[len(sbclookup.Queries)-1].Sql
	assert.Equal(t, wantQuery, lastQuery, "Got: %v. Want: %v", lastQuery, wantQuery)

	_, err = executor.Execute(ctx, nil, nil, "TestExecute", session, fmt.Sprintf("show indexes from unknown from %v", KsTestUnsharded), nil)
	require.NoError(t, err)
	lastQuery = sbclookup.Queries[len(sbclookup.Queries)-1].Sql
	assert.Equal(t, wantQuery, lastQuery, "Got: %v. Want: %v", lastQuery, wantQuery)

	// SHOW EXTENDED {INDEX | INDEXES | KEYS}
	_, err = executor.Execute(ctx, nil, nil, "TestExecute", session, fmt.Sprintf("show extended index from unknown from %v", KsTestUnsharded), nil)
	require.NoError(t, err)
	lastQuery = sbclookup.Queries[len(sbclookup.Queries)-1].Sql
	assert.Equal(t, wantQuery, lastQuery, "Got: %v. Want: %v", lastQuery, wantQuery)

	_, err = executor.Execute(ctx, nil, nil, "TestExecute", session, fmt.Sprintf("show extended indexes from unknown from %v", KsTestUnsharded), nil)
	require.NoError(t, err)
	lastQuery = sbclookup.Queries[len(sbclookup.Queries)-1].Sql
	assert.Equal(t, wantQuery, lastQuery, "Got: %v. Want: %v", lastQuery, wantQuery)

	_, err = executor.Execute(ctx, nil, nil, "TestExecute", session, fmt.Sprintf("show extended keys from unknown from %v", KsTestUnsharded), nil)
	require.NoError(t, err)
	lastQuery = sbclookup.Queries[len(sbclookup.Queries)-1].Sql
	assert.Equal(t, wantQuery, lastQuery, "Got: %v. Want: %v", lastQuery, wantQuery)
	...
	_, err = executor.Execute(ctx, nil, nil, "TestExecute", session, "show create table unknown", nil)
	require.NoError(t, err)

	_, err = executor.Execute(ctx, nil, nil, "TestExecute", session, "show full columns from table1", nil)
	require.NoError(t, err)

	// Reset target string so other tests dont fail.
	session.TargetString = "@primary"
	_, err = executor.Execute(ctx, nil, nil, "TestExecute", session, fmt.Sprintf("show full columns from unknown from %v", KsTestUnsharded), nil)
	require.NoError(t, err)
	...
	for _, sql := range []string{"show session status", "show session status like 'Ssl_cipher'"} {
		qr, err = executor.Execute(ctx, nil, nil, "TestExecute", session, sql, nil)
		require.NoError(t, err)
		wantqr = &sqltypes.Result{
			Fields: []*querypb.Field{
	...
	_, err = executor.Execute(ctx, nil, nil, "TestExecute", session, fmt.Sprintf("show full columns from %v.table1", KsTestUnsharded), nil)
	require.NoError(t, err)
...
func TestExecutorShowTargeted(t *testing.T) {
	...
	for _, sql := range queries {
		_, err := executor.Execute(ctx, nil, nil, "TestExecutorShowTargeted", session, sql, nil)
		require.NoError(t, err)
...
func TestExecutorUse(t *testing.T) {
	...
	_, err := executor.Execute(ctx, nil, nil, "TestExecute", NewSafeSession(&vtgatepb.Session{}), "use 1", nil)
	wantErr := "syntax error at position 6 near '1'"
	if err == nil || err.Error() != wantErr {
		t.Errorf("got: %v, want %v", err, wantErr)
	}

	_, err = executor.Execute(ctx, nil, nil, "TestExecute", NewSafeSession(&vtgatepb.Session{}), "use UnexistentKeyspace", nil)
...
func TestExecutorComment(t *testing.T) {
	...
	for _, stmt := range stmts {
		gotResult, err := executor.Execute(ctx, nil, nil, "TestExecute", NewSafeSession(&vtgatepb.Session{TargetString: KsTestUnsharded}), stmt, nil)
		if err != nil {
			t.Error(err)
		}
	...
	for _, stmt := range stmts {
		for _, tc := range tcs {
			t.Run(fmt.Sprintf("%s-%s", stmt, tc.targetStr), func(t *testing.T) {
				...
				_, err := executor.Execute(ctx, nil, nil, "TestExecute", NewSafeSession(&vtgatepb.Session{TargetString: tc.targetStr}), stmt, nil)
...
func TestExecutorDDL(t *testing.T) {
	...
	for _, stmt := range stmts2 {
		...
		sbc2.ExecCount.Store(0)
		sbclookup.ExecCount.Store(0)
		_, err := executor.Execute(ctx, nil, nil, "TestExecute", NewSafeSession(&vtgatepb.Session{TargetString: ""}), stmt.input, nil)
		if stmt.hasErr {
	...
	for _, stmt := range stmts {
		for _, fkMode := range []string{"allow", "disallow"} {
			t.Run(stmt+fkMode, func(t *testing.T) {
				...
				_, err := executor.Execute(ctx, nil, nil, mName, NewSafeSession(&vtgatepb.Session{TargetString: KsTestUnsharded}), stmt, nil)
				if fkMode == "allow" {
					require.NoError(t, err)
...
func TestExecutorVindexDDLACL(t *testing.T) {
	...
	_, err := executor.Execute(ctxRedUser, nil, nil, "TestExecute", session, stmt, nil)
	require.EqualError(t, err, `User 'redUser' is not authorized to perform vschema operations`)

	_, err = executor.Execute(ctxBlueUser, nil, nil, "TestExecute", session, stmt, nil)
...
func TestExecutorUnrecognized(t *testing.T) {
	...
	executor, _, _, _, ctx := createExecutorEnv(t)
	_, err := executor.Execute(ctx, nil, nil, "TestExecute", NewSafeSession(&vtgatepb.Session{}), "invalid statement", nil)
...
func TestExecutorMaxPayloadSizeExceeded(t *testing.T) {
	...
	for _, query := range testMaxPayloadSizeExceeded {
		_, err := executor.Execute(context.Background(), nil, nil, "TestExecutorMaxPayloadSizeExceeded", session, query, nil)
	...
	for _, query := range testMaxPayloadSizeOverride {
		_, err := executor.Execute(context.Background(), nil, nil, "TestExecutorMaxPayloadSizeWithOverride", session, query, nil)
...
func TestOlapSelectDatabase(t *testing.T) {
	...
	err := executor.StreamExecute(context.Background(), nil, nil, "TestExecute", NewSafeSession(session), sql, nil, cb)
	assert.NoError(t, err)
...
func TestExecutorClearsWarnings(t *testing.T) {
	...
	_, err := executor.Execute(context.Background(), nil, nil, "TestExecute", session, "select 42", nil)
	require.NoError(t, err)
...
func TestServingKeyspaces(t *testing.T) {
	...
	result, err := executor.Execute(ctx, nil, nil, "TestServingKeyspaces", NewSafeSession(&vtgatepb.Session{}), "select keyspace_name from dual", nil)
	require.NoError(t, err)
	...
	for _, stmt := range stmts {
		for _, tc := range tcs {
			t.Run(stmt+tc.targetStr, func(t *testing.T) {
				...
				_, err := executor.Execute(context.Background(), nil, nil, "TestExecute", NewSafeSession(&vtgatepb.Session{TargetString: tc.targetStr}), stmt, nil)
				if tc.hasNoKeyspaceErr {
...
func TestExecutorCallProc(t *testing.T) {
	...
	for _, tc := range tcs {
		t.Run(tc.name, func(t *testing.T) {
			...
			_, err := executor.Execute(context.Background(), nil, nil, "TestExecute", NewSafeSession(&vtgatepb.Session{TargetString: tc.targetStr}), "CALL proc()", nil)
			if tc.hasNoKeyspaceErr {
				assert.EqualError(t, err, errNoKeyspace.Error())
...
func TestExecutorTempTable(t *testing.T) {
	...
	session := NewSafeSession(&vtgatepb.Session{TargetString: KsTestUnsharded})
	_, err := executor.Execute(ctx, nil, nil, "TestExecutorTempTable", session, creatQuery, nil)
	require.NoError(t, err)
	...
	_, err = executor.Execute(ctx, nil, nil, "TestExecutorTempTable", session, "select * from temp_t", nil)
	require.NoError(t, err)
...
func TestExecutorShowVitessMigrations(t *testing.T) {
	...
	session := NewSafeSession(&vtgatepb.Session{TargetString: "TestExecutor"})
	_, err := executor.Execute(ctx, nil, nil, "", session, showQuery, nil)
	require.NoError(t, err)
...
func TestExecutorVExplainQueries(t *testing.T) {
	...
	session = NewAutocommitSession(&vtgatepb.Session{})
	err = executor.StreamExecute(ctx, nil, nil, "TestExecutorVExplainQueries", session, "vexplain queries select * from user where name = 'apa'", nil, func(result *sqltypes.Result) error {
...
func TestExecutorStartTxnStmt(t *testing.T) {
	...
	for _, tcase := range tcases {
		t.Run(tcase.beginSQL, func(t *testing.T) {
			_, err := executor.Execute(ctx, nil, nil, "TestExecutorStartTxnStmt", session, tcase.beginSQL, nil)
			require.NoError(t, err)

			assert.Equal(t, tcase.expTxAccessMode, session.GetOrCreateOptions().TransactionAccessMode)

			_, err = executor.Execute(ctx, nil, nil, "TestExecutorStartTxnStmt", session, "rollback", nil)
			require.NoError(t, err)
...
func TestExecutorPrepareExecute(t *testing.T) {
	...
	// prepare statement.
	_, err := executor.Execute(context.Background(), nil, nil, "TestExecutorPrepareExecute", session, "prepare prep_user from 'select * from user where id = ?'", nil)
	require.NoError(t, err)
	...
	_, err = executor.Execute(context.Background(), nil, nil, "TestExecutorPrepareExecute", session, "set @udv_query = 'select * from user where id in (?,?,?)'", nil)
	require.NoError(t, err)

	_, err = executor.Execute(context.Background(), nil, nil, "TestExecutorPrepareExecute", session, "prepare prep_user2 from @udv_query", nil)
	require.NoError(t, err)
	...
	_, err = executor.Execute(context.Background(), nil, nil, "TestExecutorPrepareExecute", session, "prepare prep_user2 from 'select'", nil)
	require.Error(t, err)
	...
	_, err = executor.Execute(context.Background(), nil, nil, "TestExecutorPrepareExecute", session, "prepare prep_user from @foo", nil)
	require.Error(t, err)
	require.Nil(t, session.PrepareStatement["prep_user"]) // prepared statement is cleared from the session.

	// empty prepared query
	_, err = executor.Execute(context.Background(), nil, nil, "TestExecutorPrepareExecute", session, "prepare prep_user from ''", nil)
	require.Error(t, err)
}
...
func TestExecutorTruncateErrors(t *testing.T) {
	...
	_, err := executor.Execute(ctx, nil, nil, "TestExecute", session, "invalid statement", nil)
	assert.EqualError(t, err, "syntax error at posi [TRUNCATED]")

	err = executor.StreamExecute(ctx, nil, nil, "TestExecute", session, "invalid statement", nil, fn)
	assert.EqualError(t, err, "syntax error at posi [TRUNCATED]")
...
func TestExecutorFlushStmt(t *testing.T) {
	...
	for _, tc := range tcs {
		t.Run(tc.query+tc.targetStr, func(t *testing.T) {
			_, err := executor.Execute(context.Background(), nil, nil, "TestExecutorFlushStmt", NewSafeSession(&vtgatepb.Session{TargetString: tc.targetStr}), tc.query, nil)
			if tc.expectedErr == "" {
				require.NoError(t, err)
			} else {
				require.Error(t, err)
...
func TestExecutorKillStmt(t *testing.T) {
	...
	for _, tc := range tcs {
		...
		t.Run("execute:"+tc.query+tc.errStr, func(t *testing.T) {
			...
			_, err := executor.Execute(context.Background(), mysqlCtx, nil, "TestExecutorKillStmt", NewAutocommitSession(&vtgatepb.Session{}), tc.query, nil)
			if tc.errStr != "" {
		...
		t.Run("stream:"+tc.query+tc.errStr, func(t *testing.T) {
			mysqlCtx := &fakeMysqlConnection{ErrMsg: tc.errStr}
			err := executor.StreamExecute(context.Background(), mysqlCtx, nil, "TestExecutorKillStmt", NewAutocommitSession(&vtgatepb.Session{}), tc.query, nil, func(result *sqltypes.Result) error {
				return nil
			})
			if tc.errStr != "" {
				require.ErrorContains(t, err, tc.errStr)
...
func exec(executor *Executor, session *SafeSession, sql string) (*sqltypes.Result, error) {
	return executor.Execute(context.Background(), nil, nil, "TestExecute", session, sql, nil)
}
