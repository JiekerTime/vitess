...
func TestExecutorAddSequenceDDL(t *testing.T) {
	...
	if _, err = executor.Execute(ctx, nil, nil, "TestExecute", session, stmt, nil); err != nil {
		t.Error(err)
	}
	time.Sleep(10 * time.Millisecond)
...
func TestExecutorAddDropVindexDDL(t *testing.T) {
	...
	_, err = executor.Execute(ctx, nil, nil, "TestExecute", session, "show vschema vindexes on TestExecutor.test", nil)
	...
	stmt = "alter vschema on nonexistent drop vindex test_lookup"
	_, err = executor.Execute(ctx, nil, nil, "TestExecute", NewSafeSession(&vtgatepb.Session{TargetString: "InvalidKeyspace"}), stmt, nil)
