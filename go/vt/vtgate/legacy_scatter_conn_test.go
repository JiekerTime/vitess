...
func testScatterConnBatchGeneric(t *testing.T, name string, f func(ctx context.Context, sc *ScatterConn, shards []string) (*sqltypes.Result, error)) {
	hc := discovery.NewFakeHealthCheck(nil)
...
func TestScatterConnBatchExecuteMulti(t *testing.T) {
	testScatterConnBatchGeneric(t, "TestScatterConnBatchExecuteMultiShard", func(ctx context.Context, sc *ScatterConn, shards []string) (*sqltypes.Result, error) {
		res := srvtopo.NewResolver(newSandboxForCells(ctx, []string{"aa"}), sc.gateway, "aa")
