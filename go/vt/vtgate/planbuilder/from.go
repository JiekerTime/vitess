...
func (pb *primitiveBuilder) buildTablePrimitive(tableExpr *sqlparser.AliasedTableExpr, tableName sqlparser.TableName) error {
	...
	if sqlparser.SystemSchema(tableName.Qualifier.String()) {
		...
		rb.eroute.TableNameSlice = append(rb.eroute.TableNameSlice, rb.eroute.TableName)
		pb.plan, pb.st = rb, st
	...
	eroute.TableNameSlice = append(eroute.TableNameSlice, eroute.TableName)
	rb.eroute = eroute
...
func (pb *primitiveBuilder) join(rpb *primitiveBuilder, ajoin *sqlparser.JoinTableExpr, reservedVars *sqlparser.ReservedVars, where sqlparser.Expr) error {
	...
	if lRoute.eroute.TableName != rRoute.eroute.TableName {
		lRoute.eroute.TableNameSlice = append(lRoute.eroute.TableNameSlice, lRoute.eroute.TableName, rRoute.eroute.TableName)
