...
func routeToEngineRoute(ctx *plancontext.PlanningContext, op *operators.Route) (*engine.Route, error) {
	...
	return &engine.Route{
		...
		TableNameSlice:      operators.TableNamesUsed(op),
...
func buildInsertLogicalPlan(ctx *plancontext.PlanningContext, rb *operators.Route, op ops.Operator, stmt *sqlparser.Insert) (logicalPlan, error) {
	...
	eins := &engine.Insert{
		Opcode:            mapToInsertOpCode(rb.Routing.OpCode(), ins.Input != nil, ins.VTable.Pinned != nil),
	...
	// pinned table
	if ins.VTable.Pinned != nil {
		eins.TargetDestination = key.DestinationKeyspaceID(ins.VTable.Pinned)
	}
...
func mapToInsertOpCode(code engine.Opcode, insertSelect bool, pinnedTable bool) engine.InsertOpcode {
	...
	if pinnedTable {
		return engine.InsertByDestination
	}
...
func buildUpdateLogicalPlan(
	...
	if upd.VTable.Pinned != nil {
		edml.TargetDestination = key.DestinationKeyspaceID(upd.VTable.Pinned)
	}
...
func buildDeleteLogicalPlan(
	...
	if del.VTable.Pinned != nil {
		edml.TargetDestination = key.DestinationKeyspaceID(del.VTable.Pinned)
	}
