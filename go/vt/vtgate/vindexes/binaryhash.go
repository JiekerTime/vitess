/*
Copyright 2017 Google Inc.

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

package vindexes

import (
	"bytes"
	"context"
	"crypto/des"
	"fmt"
	"strconv"
	"strings"
	"vitess.io/vitess/go/sqltypes"
	"vitess.io/vitess/go/vt/key"
)

var (
	_ Vindex     = (*BinaryHash)(nil)
	_ Reversible = (*BinaryHash)(nil)
)

// BinaryHash defines vindex that hashes an int64 to a KeyspaceId
// by using null-key 3DES hash. It's Unique, Reversible and
// Functional.
type BinaryHash struct {
	name string
}

// NewBinaryHash creates a new Hash.
func NewBinaryHash(name string, m map[string]string) (Vindex, error) {
	return &BinaryHash{name: name}, nil
}

// String returns the name of the vindex.
func (vind *BinaryHash) String() string {
	return vind.name
}

// Cost returns the cost of this index as 1.
func (vind *BinaryHash) Cost() int {
	return 1
}

// IsUnique returns true since the Vindex is unique.
func (vind *BinaryHash) IsUnique() bool {
	return true
}

// IsFunctional returns true since the Vindex is functional.
func (vind *BinaryHash) IsFunctional() bool {
	return true
}

// Map can map ids to key.Destination objects.
func (vind *BinaryHash) Map(ctx context.Context, cursor VCursor, ids []sqltypes.Value) ([]key.Destination, error) {
	out := make([]key.Destination, len(ids))
	for i, id := range ids {
		var num uint64
		var err error

		if id.IsSigned() {
			// This is ToUint64 with no check on negative values.
			str := id.ToString()
			var ival int64
			ival, err = strconv.ParseInt(str, 10, 64)
			num = uint64(ival)
		} else {
			num, err = id.ToUint64()
		}

		if err != nil {
			h64 := New64()
			_, err = h64.Write([]byte(strings.ToLower(strings.TrimSpace(string(id.Raw())))))
			if err != nil {
				out[i] = key.DestinationNone{}
				continue
			}
			num = h64.Sum64()
		}

		out[i] = key.DestinationKeyspaceID(vhash(num))
	}
	return out, nil
}

// Verify returns true if ids maps to ksids.
func (vind *BinaryHash) Verify(_ context.Context, _ VCursor, ids []sqltypes.Value, ksids [][]byte) ([]bool, error) {
	out := make([]bool, len(ids))
	for i := range ids {
		num, err := ids[i].ToUint64()
		if err != nil {
			return nil, fmt.Errorf("hash.Verify: %v", err)
		}
		out[i] = bytes.Equal(vhash(num), ksids[i])
	}
	return out, nil
}

// ReverseMap returns the ids from ksids.
func (vind *BinaryHash) ReverseMap(_ VCursor, ksids [][]byte) ([]sqltypes.Value, error) {
	reverseIds := make([]sqltypes.Value, 0, len(ksids))
	for _, keyspaceID := range ksids {
		val, err := vunhash(keyspaceID)
		if err != nil {
			return reverseIds, err
		}
		reverseIds = append(reverseIds, sqltypes.NewUint64(val))
	}
	return reverseIds, nil
}

// NeedsVCursor satisfies the Vindex interface.
func (vind *BinaryHash) NeedsVCursor() bool {
	return false
}

func init() {
	var err error
	blockDES, err = des.NewTripleDESCipher(make([]byte, 24))
	if err != nil {
		panic(err)
	}
	Register("binaryhash", NewBinaryHash)
}
