...
func (vtg *VTGate) Execute(ctx context.Context, mysqlCtx vtgateservice.MySQLConnection, c *mysql.Conn, session *vtgatepb.Session, sql string, bindVariables map[string]*querypb.BindVariable) (newSession *vtgatepb.Session, qr *sqltypes.Result, err error) {
	...
	if bvErr := sqltypes.ValidateBindVariables(bindVariables); bvErr != nil {
		...
		qr, err = vtg.executor.Execute(ctx, mysqlCtx, c, "Execute", safeSession, sql, bindVariables)
...
func (vtg *VTGate) ExecuteBatch(ctx context.Context, c *mysql.Conn, session *vtgatepb.Session, sqlList []string, bindVariablesList []map[string]*querypb.BindVariable) (*vtgatepb.Session, []sqltypes.QueryResponse, error) {
	// In this context, we don't care if we can't fully parse destination
	destKeyspace, destTabletType, _, _ := vtg.executor.ParseDestinationTarget(session.TargetString)
	...
	for i, sql := range sqlList {
		...
		session, qrl[i].QueryResult, qrl[i].QueryError = vtg.Execute(ctx, nil, c, session, sql, bv)
...
func (vtg *VTGate) StreamExecute(ctx context.Context, mysqlCtx vtgateservice.MySQLConnection, c *mysql.Conn, session *vtgatepb.Session, sql string, bindVariables map[string]*querypb.BindVariable, callback func(*sqltypes.Result) error) (*vtgatepb.Session, error) {
	// In this context, we don't care if we can't fully parse destination
	destKeyspace, destTabletType, _, _ := vtg.executor.ParseDestinationTarget(session.TargetString)
