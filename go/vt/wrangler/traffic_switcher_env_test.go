...
const (
	...
	streamExtInfoQuery = "select id, source, pos, stop_pos, max_replication_lag, state, db_name, time_updated, transaction_timestamp, time_heartbeat, time_throttled, component_throttled, message, tags, workflow_type, workflow_sub_type, defer_secondary_keys, rows_copied from _vt.vreplication where db_name = '%s' and workflow = '%s'"
...
func (tme *testShardMigraterEnv) expectStartReverseVReplication() {
	...
	// The ids returned are not accurate.
	for _, dbclient := range tme.dbSourceClients {
		dbclient.addQuery("select id from _vt.vreplication where db_name = 'ks' and workflow = 'test_reverse'", resultid34, nil)
